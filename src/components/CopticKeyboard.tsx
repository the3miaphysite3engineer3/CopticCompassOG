...
export default function CopticKeyboard({ onAppend, onBackspace, isOpen, onClose }: CopticKeyboardProps) {
  ...
  return (
    <div className="absolute top-[calc(100%+0.75rem)] right-0 z-50 w-full md:w-[640px] bg-white/95 dark:bg-stone-900/92 backdrop-blur-xl border border-stone-200 dark:border-stone-700/80 rounded-3xl p-4 md:p-5 shadow-2xl">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-xs text-stone-400 dark:text-stone-500 mt-1">Insert Coptic letters and diacritics directly into search.</p>
        </div>
        <button onClick={onClose} className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-stone-100 text-stone-500 hover:text-stone-700 dark:bg-stone-800/70 dark:text-stone-400 dark:hover:text-stone-200 transition-colors">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="flex gap-2.5 mb-2.5">
        {DIACRITICS.map(d => (
          <button
            ...
            className="flex-1 h-11 flex items-center justify-center font-coptic text-lg text-stone-700 dark:text-stone-300 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 rounded-xl border border-emerald-200 dark:border-emerald-900/50 transition-colors active:scale-95"
